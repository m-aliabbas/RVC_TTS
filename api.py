...
default_sid = (
    get_speakers_id(default_model_file)[0] if get_speakers_id(default_model_file) else 0
)
...
async def tts_endpoint(request: TTSRequest):
    try:
        ...
        output_tts_path = os.path.join(
            os.getcwd(), "assets", "audios", f"{ran_file_name}_tts_output.wav"
        )
        output_rvc_path = os.path.join(
            os.getcwd(), "assets", "audios", f"{ran_file_name}_tts_rvc_output.wav"
        )
        ...
        raise HTTPException(
            status_code=500, detail=f"Error during TTS conversion: {str(e)}"
        )
